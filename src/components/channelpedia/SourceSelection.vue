<template>
  <v-sheet color="#0d1117" elevation="3" rounded="lg">
    <v-tabs v-model="selectedSource" align-tabs="center" color="white">
      <v-tab
    ...
    >
      <template #item="{ props, item }">
        <v-list-item v-bind="props" :title="getLocationTitle(item.title)">
          <template v-if="item.title[2] === '_'" #prepend>
            {{ getFlagEmoji(item.title.slice(0, 2)) }}
            <v-divider vertical thickness="10" class="border-opacity-0" />
          </template>
  ...
  <v-sheet class="overflow-y-auto" max-height="85dvh">
    <v-expansion-panels v-if="subGroupItems" variant="accordion">
      <template v-for="subGroupItem in subGroupItems" :key="subGroupItem.id">
        <v-expansion-panel v-if="subGroupItem" :title="subGroupItem.title">
          <v-expansion-panel-text>
            <ChannelCandidateList
              :original-channel-candidates="
                subGroupItem.children as unknown as VDRChannel[]
              "
              ...
              "
              @insert-channel="
                (channel, number, scroll) => {
                  $emit('insertChannel', channel, number, scroll);
                }
              "
            />
          </v-expansion-panel-text>
...
import type {
  ChannelSubGroup,
  VDRChannel,
} from "@/stores/interfaces/VdrChannelInterface";
...
const channelpediaSubGroupMap: Ref<Map<string, ChannelSubGroup[]>> = ref(
  new Map(),
);
...
watch([selectedSource], () => {
  if (locationItems.value && locationItems.value.length > 0) {
    const locationItem = locationItems.value[0];
    if (locationItem) {
      selectedLocation.value = locationItem.title;
...
watch([selectedGroup], () => {
  const selectedEntry = groupItems.value?.find(
    (item) => item.title === selectedGroup.value,
  );
...
onMounted(async () => {
  try {
    ...
    if (locationItems.value && locationItems.value.length > 0) {
      const targetLocation = locationItems.value[0];
      if (targetLocation) selectedLocation.value = targetLocation.title;
    }
  } catch (error) {
