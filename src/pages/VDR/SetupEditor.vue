...
const visibleEntries = computed(()=> {
  ...
  if (searchText.value?.length > 0) {
    return setupEntries.value.filter((entry) => entry.name.toLocaleLowerCase().search(searchText.value.toLowerCase()) >= 0)
  } else {
...
async function loadVDRSetup() {
  ...
  try {
    const data = await store.getRequest('/vdr/setup')
    setupEntries.value = data 
  } catch (error) {
...
async function reloadValue(name: string) {
  try {
    const value = await store.getRequest(`/vdr/setup?key=${encodeURIComponent(name)}`)
    ...
    console.log("updated value for: ", name, ': ', value, 'at index', idx)
