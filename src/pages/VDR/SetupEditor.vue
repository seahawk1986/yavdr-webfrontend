<template>
  ...
  >
    ...
    >
      <v-card-title id="innerTitle" class="flex-none">
        <div class="d-flex align-center justify-space-between w-100">
          <h2 class="text-h5">VDR Setup</h2>
          <v-btn
            v-tooltip="t('actions.reload')"
            color="primary"
      ...
      <v-card-text style="position: relative; min-height: 0">
        <v-text-field
          v-model="searchText"
        ...
        <v-dialog v-model="showEditDialog" max-width="500">
          <v-card v-if="selectedItem">
            <v-card-title>{{
              t("actions.edit", {
                what: t("category.setupValueFor", {
                  what: `'${selectedItem.name}'`,
                }),
              })
            }}</v-card-title>
            <v-card-text>
              ...
              <v-switch
                v-if="
                  selectedItem.min_value == 0 && selectedItem.max_value == 1
                "
                ...
                :rules="[
                  (value: string) => {
                    if (
                      ...
                      selectedItem?.max_value > 0
                    ) {
                      if (value.length > selectedItem?.max_value)
                        return `Value longer than ${selectedItem?.max_value}`;
                    }
                    return false;
                  },
                ]"
...
//       if (setupEntry?.value) setupEntry.value = response.data;
...
async function sendValue(name: string, value: string | number) {
  console.log("saved value for: ", name, ": ", value);
  try {
