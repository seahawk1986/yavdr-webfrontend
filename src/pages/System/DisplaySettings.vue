    ...
    >
      <v-card color="grey-darken-4">
        ...
        <v-card-actions>
          ...
          <v-btn
            color="primary"
            :disabled="isLookingForDisplays || isSettingConfig"
...
const convert = new AnsiConverter({
  ...
  bg: "#000",
  newline: true,
  escapeXML: true, // Sicherheit gegen XSS (sehr wichtig bei v-html!)
  stream: true,
});
...
const updateDisplayConfig = async () => {
  const xorg_config_response = await backend.getRequest(
...
const primaryDisplayResolutionOptions = computed(() => {
  const key = primaryDisplay?.value;

  if (!key) return [];
  const display = displayOutputs?.value?.[key];

  if (!display?.modes) return [];

  return Object.keys(display.modes).sort((a, b) => {
...
const secondaryDisplayResolutionOptions = computed(() => {
  ...
  const key = secondaryDisplay?.value;
...
const secondaryDisplayRefreshrateOptions = computed(() => {
  const secondary = secondaryDisplay?.value;
  if (!secondary) return [];

  const secondary_display = displayOutputs?.value?.[secondary];
  if (!secondary_display || !secondaryDisplayResolution.value) return [];
  const refreshrates =
    secondary_display.modes[secondaryDisplayResolution.value];

  return refreshrates ? refreshrates : [];
});
...
function processPlaybookOutput(data: AnsibleJobEventInterface) {
  ...
  if (data.event?.stdout && data.event.stdout.length > 0) {
    ...
    playbookLogs.value.push(
      `<span class="text-info">Status: ${data.status.status}</span>`,
    );
  }
}
...
async function scanDisplays() {
  ...
  try {
    ...
    playbookLogs.value.push(
      `<span class="text-error">Fehler: ${errorMessage}</span>`,
    );
  } finally {
    await updateDisplayConfig();
...
watch(secondaryDisplay, async (newValue, oldValue) => {
  ...
  if (
    ...
    console.log(
      "xorgConfig.secondary:",
      displayConfig.value.secondary?.connector,
    );
    // set the resolution and refreshrate according to the existing configuration if possible
    if (newValue === displayConfig.value.primary?.connector) {
